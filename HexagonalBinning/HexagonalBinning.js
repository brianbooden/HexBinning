define(["jquery", "text!./HexagonalBinning.css", "./d3.min", "./hexbin", "./lasso_adj", "./senseUtils"], function ($, cssContent) {
    'use strict';
...
function drawHex($element, layout, fullMatrix, self) {
    ...
    console.log(qMatrix);
    var data = qMatrix.filter(function (d) {
        return (d[0].qElemNumber >= 0 && d[1].qNum != "NaN" && d[2].qNum != "NaN");
    }).map(function (d) {
...
var viz = function (self, data, labels,
    ...
    if (useStaticLayout) {
        ...
        // Set the y-axis to min and max of Metric 2
        var xExt = d3.extent(data, function (d) {
            return d.Metric1;
        });
        ...
        // Set the y-axis to min and max of Metric 2	
        var yExt = d3.extent(data, function (d) {
            return d.Metric2;
        });
    ...
    var maxBinSize = d3.max(hexBin, function (d) {
        return d.length;
    });
    ...
    if (binningMode == 0) {
        ...
        if (colorAxis == 2) {
            ...
            if (useStaticLayout) {
                ...
                var colorScale = d3.scale.quantile()
                    .domain([(colorAxis == 0 ? measureMin1 : measureMin2), d3.mean(data, function (d) {
                        return (colorAxis == 0 ? d.Metric1 : d.Metric2);
                    }), (colorAxis == 0 ? measureMax1 : measureMax2)])
                    .range(colorpalette);
            }
        }
        ...
        var hexpoints2 = hexpoints.enter().append("path")
            ...
            .attr("binCnt", function (d) {
                return d.length;
            })
            .attr("id", function (d) {
                return id + "_" + d[0][3];
            }) // use Dim1_Key as Path ID
            .attr("transform", function (d) {
                return "translate(" + d.x + "," + d.y + ")";
            })
            .style("fill", function (d) {
                return colorScale((colorAxis == 2 ? d.length : d.reduce(function (sum, a, i, ar) {
                    sum += a[colorAxis];
    ...
    if (titleLayout == 0) {
        ...
        hexpoints2.append("title").text(function (d) {
            return (d.length > 20 ? (d.length > 5 ? d.length + " members:\n" : "") + d.slice(0, 20).map(function (e) {
                return e[2] + ": " + e[0] + " / " + e[1];
            }).join("\n") + "\n+" + (d.length - 20) + " members" : (d.length > 5 ? d.length + " members:\n" : "") + d.map(function (e) {
                return e[2] + ": " + e[0] + " / " + e[1];
            }).join("\n"));
        });
    }
